  ...
  var Meny = {
    ...
    create: function (options) {
      return (function () {
        ...
        if (
          options.menuElement.parentNode !== options.contentsElement.parentNode
        ) {
          ...
          switch (config.position) {
            ...
            case POSITION_R:
              ...
              menuTransformClosed =
                "rotateY( " +
                menuAngle +
                "deg ) translateX( 100% ) translateX( -2px ) scale( 1.01 )";
        ...
        function setupCover() {
          ...
          try {
            ...
            dom.cover.style.background =
              "-ms-linear-gradient(" + config.position + "," + config.gradient;
            dom.cover.style.background =
              "-moz-linear-gradient(" + config.position + "," + config.gradient;
            dom.cover.style.background =
          ...
          if (supports3DTransforms) {
            dom.cover.style[Meny.prefix("transition")] =
              "all " +
        ...
        function open() {
          if (!isOpen) {
            ...
            // ...fall back on JS animation
              ...
              contentsAnimation = Meny.animate(
                dom.contents,
                contentsStyleOpened,
                500
              );
        ...
        function destroy() {
          ...
          for (var i in addedEventListeners) {
            this.removeEventListener(
              addedEventListeners[i][0],
              addedEventListeners[i][1]
            );
          }
        ...
        function onTouchMove(event) {
          ...
          if (
            Math.abs(touchMoveX - touchStartX) >
            Math.abs(touchMoveY - touchStartY)
          ) {
        ...
        function onTap() {
          var isOverContent =
            (config.position === POSITION_T && touchStartY > config.height) ||
            (config.position === POSITION_R &&
              touchStartX < dom.wrapper.offsetWidth - config.width) ||
            (config.position === POSITION_B &&
              touchStartY < dom.wrapper.offsetHeight - config.height) ||
      ...
      return (function () {
        ...
        for (var p in properties) {
          interpolations[p] = {
            ...
            unit:
              typeof properties[p] === "string" &&
              properties[p].match(/px|em|%/gi)
        ...
        function step() {
          // Ease out
          var progress =
            1 - Math.pow(1 - (Date.now() - animationStartTime) / duration, 5);
          ...
          for (var p in interpolations) {
            ...
            element.style[p] =
              property.start +
              (property.end - property.start) * progress +
              property.unit;
          }
    ...
    prefix: function (property, el) {
      ...
      for (var i = 0, len = vendors.length; i < len; i++) {
        ...
        if (
          typeof (el || document.body).style[vendor + propertyUC] !==
          "undefined"
        ) {
